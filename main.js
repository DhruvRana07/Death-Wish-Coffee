...
function addToCart(id){
        fetch(`https://project-json-ezbg.onrender.com/data/${id}`).then((res)=>res.json()).then((data)=>{
            let addObj = {
                "image": data.image,
                "title": data.title,
                "price": data.price,
            ...
            fetch("https://project-json-ezbg.onrender.com/addToCartData", {
                method: "POST",
                headers: {
...
function secondAddCart(id){
    fetch(`https://project-json-ezbg.onrender.com/seller-Data/${id}`).then((res)=>res.json()).then((data2)=>{
            let addObj2 = {
                "image": data2.image,
                "title": data2.title,
                "price": data2.price,
...
function fetchaddData(){
    fetch(`https://project-json-ezbg.onrender.com/addToCartData`).then((resp)=>resp.json()).then((addData)=>{
...
function deleteformAddtoCart(id){
    fetch(`https://project-json-ezbg.onrender.com/addToCartData/${id}`, {
        method: "DELETE",
    },setTimeout(() => {
        window.location.reload(true)
    }, 500))
...
function addToCartBox(id){
    fetch(`https://project-json-ezbg.onrender.com/coffee_data/${id}`).then((res)=>res.json()).then((data)=>{
        let coffeeData ={
...
function addToCartMerchData(id){
    fetch(`https://project-json-ezbg.onrender.com/merch_Data/${id}`).then((res)=>res.json()).then((merchdata)=>{
        let merchData ={
...
function addToCartBundleData(id){
    console.log(id)
    fetch(`https://project-json-ezbg.onrender.com/bundles_Data/${id}`).then((res)=>res.json()).then((bundledata)=>{
        let bundleData ={
